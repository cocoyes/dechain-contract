...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    // item title
    mapping (uint256 => string) public _tokenTitles;

    // item content
    mapping (uint256 => string) public _tokenContents;
    ...
    */
    ...
    function _setTokenTitle(uint256 tokenId, string memory _tokenTitle) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenTitles[tokenId] = _tokenTitle;
    }
    function _setTokenContent(uint256 tokenId, string memory _tokenContent) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenContents[tokenId] = _tokenContent;
    }
...
contract NFT is ERC721, Ownable {
    ...
    string public coverPic;
    string public coverIntr;
    ...
    constructor(string memory tokenName,string memory tokenSymbol,string memory _coverPic,string memory _coverIntr) ERC721(tokenName, tokenSymbol) {
        _contractOwner = msg.sender;
        coverPic=_coverPic;
        coverIntr=_coverIntr;
    }

    function mint(string memory _tokenURI, address _toAddress, uint _price,string memory title,string memory content) public returns (uint) {
        ...
        _setTokenTitle(_tokenId,title);
        _setTokenContent(_tokenId,content);
    ...
    function getTokenItemInfo(uint _tokenId) public view returns(uint _price,address _account,bool _canTrade,string memory _pic,string memory _title,string memory _content){
        address account= ownerOf(_tokenId);
        bool st=listedMap[_tokenId];
        uint pr=price[_tokenId];
        string memory tit=_tokenTitles[_tokenId];
        string memory con=_tokenContents[_tokenId];
        string memory url=tokenURI(_tokenId);
        return (pr,account,st,url,tit,con);
    }
}
