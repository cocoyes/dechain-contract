...
contract Redpack {
    ...
    function toll(uint count,uint value,string memory  id) public payable {
        ...
        uint total=value;
        while(loop<count){
            max = total - min * (count - loop);
            uint k = (uint)(count - loop) / 2;
            if (count - loop <= 2) {
                k = count - loop;
            }
            max = max / k;
            uint money = uint(keccak256(abi.encodePacked(now, msg.sender, loop))) %max;
            playerInfo[id].randomAmount[loop-1]=money;
            total=total-money;
            loop++;
            if (loop==count){
                playerInfo[id].randomAmount[count-1]=total;
            }
        }
    ...
    function hunting(string memory  id) public payable {
        ...
        uint value = playerInfo[id].randomAmount[index];
        hunted(id, value);
        playerInfo[id].balance -= value;
    }
